...
%\VignetteIndexEntry{array-refcard: Array operations in gRbase}
...
\renewenvironment{knitrout}{\small\renewcommand{\baselinestretch}{.7}}{} 


<<echo=F>>=
options("digits"=4)
options(useFancyQuotes = FALSE)
##options(SweaveHooks=list(fig=function() par(mar=c(2,2,2,2))))
rm(list=ls())
...
\begin{center}
     \Large{\textbf{\color{blue}Array Operations Reference Card for \grbase}} \\
\end{center}

by S{\o}ren H{\o}jsgaard, sorenh@math.aau.dk
...
An array with dimnames and whose dimnames are named (as the case is above) is in
\grbase\ called a ``named array''. Notice that \grbase\ imports the pipe operator
\verb'%>%' operator. For compact printing we shall use:

<<>>= 
flat <- function(x) {ftable(x, row.vars=1)}
@ %def 

An example of a named array is:
...
\section{Operations on named arrays}
\label{sec:oper-named-arrays}

Most functions on named arrays in \grbase\ have names of the form
\texttt{arsomething}.  


\section{Unary operations}
\label{sec:unary-operations}


Extract slice of array:
<< >>= 
arslice(HairEyeColor, 
               list(Hair=c("Black","Brown"), 
                    Eye=c("Brown","Blue","Hazel")))  %>% flat
@

<<results='hide'>>= 
arslice(HairEyeColor, list(Hair=1:2, Eye=1:3))
@

Permute entries of array:
<< >>= 
arperm(hec, c("Eye","Hair","Sex"))  %>% flat
@

<<results='hide'>>= 
arperm(hec, ~Eye:Hair:Sex)
arperm(hec, c(2,1,3))
@

Collapsing (marginalizing) an array:
<< >>= 
armarg(hec, ~Hair:Eye) %>% flat
@

<<results='hide' >>= 
armarg(hec, c("Hair","Eye")) 
armarg(hec, 1:2) 
@

Inflating an array:
<<>>=
z <- armarg(hec, ~Hair:Eye)
arexpand(z, list(A=c("a1","a2")))  %>% flat
@ %def 
...
All 2--way tables:
<<>>=
he <- armarg(hec, ~Hair:Eye)
hs <- armarg(hec, c("Hair","Sex"))
es <- armarg(hec, c(2,3))
@ %def 
...
\section{Defining arrays}
\label{sec:defining-arrays}
...
z2 <- array( c(32, 53, 11, 50, 10, 25, 36, 66, 9, 34, 5, 29),
            dim=di, dimnames=dn)
@ %def
where the \code{dimnames} part in both cases is optional. However, to qualify as
a named array, the dimnames are necessary.
Another way is to use \texttt{newar()} from \grbase:
@ 
<<>>=
...
Notice that \code{dn} in \code{newar()} is allowed to contain superfluous elements. Default
...
<<>>=
he
arslice_entries(he, slice=list(Hair=1))
arslice_mult(he, slice=list(Hair=1), val=10, comp=-1)
ardist(he)
ardist(he, cond=~Hair)
@ %def 
