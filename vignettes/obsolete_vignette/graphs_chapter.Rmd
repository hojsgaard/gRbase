...
\em As of major version 2 of \texttt{gRbase}, that is versions
 2.x.y, \texttt{gRbase} no longer depends on the packages
 \verb|graph|, \verb|Rgraphviz|, and \verb|RBGL| packages.  Graph
 functionality in these packages now relies either on the
 \verb|igraph| package or on graph algorithms implemented in
 \verb|gRbase|. This document reflects these changes.
...
* This document also reflects that since GMwR was published in 2012,
  ...
  includes the packages \verb|lcd| and \verb|sna|.
...
* In this document it has been emphasized if a function has been
 ...
 \texttt{gRbase} by writing \texttt{'igraph::this\_function()'} and
 \texttt{'gRbase::this\_function()'}
...
The graph $\calG_0=(V_0,E_0)$ is said to be a *subgraph* of
...
A *directed graph* as a mathematical object is a pair $\calG
...
The *components* of a chain graph $\calG$ are the connected
...
*component DAG* of $\calG$, written $\calG_{C}$.
...
in $\calG_{C}$. The *anterior graph* of $S \subseteq V$ is the subgraph of
...
A triple $(A,B,D)$ of non--empty disjoint subsets of $V$ is said to *decompose* $\calG$ into
