...
//' @param plevels A vector used for fast computations of entries from cells; generated by 'make_plevels_'.
...
int get_cell_number_prim_(const NumericVector& cell, const IntegerVector& perm, const IntegerVector& plevels){
...
IntegerVector perm_cell_entries_(const IntegerVector& perm, const IntegerVector& dim){
  ...
  for (i=0; i < len_entry; i++){
    //Rf_PrintValue( cell );
    out = get_cell_number_prim_(cell, perm, plevels);
    entry_new[i] = out;
